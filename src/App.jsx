...
function StarIcon({ filled, hovered }) {
  return (
    <svg viewBox="0 0 24 24" fill={filled || hovered ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" style={{ opacity: filled ? 1 : hovered ? 0.4 : 0.3 }}>
...
function App() {
  ...
  const needsSubmitButton = current.type === 'multi' || current.type === 'text' || current.type === 'slider'
  ...
  }, [current.id, currentStep, isAnimating, transitionToStep])
  ...
  // Emoji select — auto advance
  const handleEmojiSelect = useCallback((label) => {
    if (isAnimating) return
    setError('')
  ...
  // Submit / next for multi, text & slider
  ...
  return (
    <div className="survey-container">
      ...
      <div className="survey-form-panel">
        <svg className="survey-logo" viewBox="0 0 303 24" fill="none" xmlns="http://www.w3.org/2000/svg">
  <g clipPath="url(#clip0_1004_668)">
    ...
    <clipPath id="clip0_1004_668">
      <rect width="302.836" height="24" fill="white" transform="translate(0.0820312)"/>
        ...
        <div className="survey-progress-wrapper">
          <div className="survey-progress-label">{currentStep + 1} of {questions.length}</div>
                ...
                <div className="stars-wrapper">
                  <div className="stars-container">
                    {Array.from({ length: current.max }, (_, i) => {
                      const val = i + 1
                      const filled = currentAnswer >= val
                      const hovered = hoveredStar >= val
                      return (
                        <button
                          key={val}
                          className={`star-btn ${filled ? 'active' : ''}`}
                          onClick={() => handleStarSelect(val)}
                  ...
                  <div className={`star-label ${(currentAnswer || hoveredStar) ? 'visible' : ''}`}>
                    {starLabels[hoveredStar || currentAnswer || 0]}
                  </div>
                </div>
              )}
              ...
              {current.type === 'emoji' && (
                <div className="emoji-container">
                  {current.options.map((opt) => (
                    <button
                      key={opt.label}
                      className={`emoji-btn ${currentAnswer === opt.label ? 'selected' : ''}`}
                      onClick={() => handleEmojiSelect(opt.label)}
                    >
              ...
              {current.type === 'slider' && (
                <div className="slider-container">
                  <div className="slider-value-display">
                    {currentAnswer || current.min}
                    <span className="slider-unit">{current.unit}</span>
                  </div>
                  <div className="slider-track-wrapper">
                    <input
                      ...
                      max={current.max}
                      step={current.step}
                      value={currentAnswer || current.min}
                      onChange={(e) => handleSliderChange(e.target.value)}
                    />
                  </div>
                  <div className="slider-labels">
                    <span className="slider-label">{current.min} {current.unit}</span>
                    <span className="slider-label">{current.max}+ {current.unit}</span>
                  </div>
                </div>
              )}
